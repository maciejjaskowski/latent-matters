...
class Net(nn.Module):
    def __init__(self, n_input_channels, classification, n_classes=1):
        ...
        self.hidden_hw = 18
        self.ran = torch.Tensor([range(self.hidden_hw)] * self.hidden_hw)
    ...
    def _softargmax1d(self, x):
        softmaxed = F.softmax(x, dim=-1)
        exp = torch.Tensor(range(softmaxed.shape[-1]))
        exp_value = torch.sum(exp * softmaxed, dim=-1)
        return exp_value
    ...
    def forward(self, X):
        ...
        # Uzyj (first- first_prev) i (second - first_prev) do oszacowania _softargmax1d(out)
        # Potem loss to jest L2[(first-first_prev) * _softmax1d(out) - (second-first_prev)]
        # Zalozenie jest takie, ze w ten sposob unikamy dzielenia, ktore jest problematyczne
        # i ten trick pomoze.

        # Dodatkowy pomysl: uzyc loss_seg - tj. penalizujemy predykcje, ktore nie wskazuja pikseli
        # ktore sie zmienily pomiedzy first i first_prev (sprawdzic w pracy z keypointami, jak to jest robione)


        # Mam wrazenie, ze tu ryzykujemy, ze informacje nam uciekna... :/
        r1 = torch.cat([first - first_prev, second - first_prev], dim=-1)
        r2 = F.relu(self.fc1(r1))
        r3 = F.relu(self.fc2(r2))
        out = self.fc3(r3)

        diff = (first - first_prev)*self._softargmax1d(out) - (second-first_prev)
        return diff
